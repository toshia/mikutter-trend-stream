...
Module.new do
  ...
  plugin = Plugin::create(:trend_streaming)
  main = Gtk::TimeLine.new()
  main.force_retrieve_in_reply_to = false
  queue_parse = Queue.new
  queue_event = TimeLimitedQueue.new(4, 1){ |messages| Delayer.new{ main.add messages } }
        ...
        when /^\{.*\}$/
          ...
          if messages.is_a? Enumerable
            messages.each{ |message|
              queue_event.push message if message.is_a? Message } end end } }
  }
end
